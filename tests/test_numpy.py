...
def json_data():
    ...
    if not os.path.exists(json_file):
        pytest.skip(f"{json_file} not found in {os.getcwd()}. Run the main script first.")
...
def test_json_structure(json_data):
    ...
    for item in json_data:
        ...
        assert set(item.keys()) == {'subdirectory', 'filename', 'type', 'original', 'updated'}, \
            f"Item has incorrect keys: {item.keys()}"
        assert item['subdirectory'] in SUBDIRS, f"Unexpected subdir {item['subdirectory']} in the JSON output"
