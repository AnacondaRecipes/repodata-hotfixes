...
def clone_subdir(channel_base_url: str, subdir: str):
    ...
    out_file = os.path.join(
        channel_base_url.rsplit("/", 1)[-1], subdir, "repodata-reference.json"
    )
    ...
    out_file = os.path.join(
        channel_base_url.rsplit("/", 1)[-1], subdir, "repodata_from_packages.json"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Test new hotfix changes before they are deployed."
    )
    parser.add_argument("channel", help="channel name or url to download repodata from")
    parser.add_argument(
        "--subdirs",
        nargs="*",
        help="subdir(s) to download/diff",
        default=(conda_subdir,),
    )
    parser.add_argument(
        "--diff-format",
        help="format to save diff as",
        choices=("unified", "context", "html"),
        default="html",
    )
    parser.add_argument(
        "--context-numlines",
        help="context lines to show around diff",
        type=int,
        default=5,
    )
    parser.add_argument("--use-cache", action="store_true", help="use cached repodata")
    parser.add_argument(
        "--color", action="store_true", help="use colordiff rather than diff"
    )
    parser.add_argument(
    ...
    print(
        f"Creating channel directory structure for channel '{args.channel}' and platforms {args.subdirs}"
    )
    ...
    for subdir in args.subdirs:
        raw_repodata_file = os.path.join(
            args.channel, subdir, "repodata_from_packages.json"
        )
        ...
        patched_repodata_file = os.path.join(
            args.channel, subdir, "repodata-patched.json"
        )
        print(
            f"Writing out new repodata as {patched_repodata_file} for '{subdir}' platform."
        )
        with open(patched_repodata_file, "w") as f:
            json.dump(
                patched_repodata, f, indent=2, sort_keys=True, separators=(",", ": ")
            )
            f.write("\n")

        ref_repodata_file = os.path.join(
            args.channel, subdir, "repodata-reference.json"
        )
        if args.show_pkgs:
            print("New Hot Fixes:")
            ...
            subprocess.call(
                [diff_exe, fmt_flag, ref_repodata_file, patched_repodata_file]
            )
