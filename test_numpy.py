...
import pytest
import warnings
warnings.filterwarnings("ignore", category=DeprecationWarning, module="boltons.timeutils")
            ...
            for change in changes:
                if change['original'] != change['updated']:
                    assert '<2.0a0' in change['updated'], \
                        f"Updated dependency for {package} in {subdir} doesn't have correct upper bound: {change['updated']}"
